...
class Bot_response(threading.Thread):
    def __init__(self):
        threading.Thread.__init__(self)
    def run(self,bot):
        chatid=update[-1].message.chat_id
        content=update[-1].message.text
        if content.startswith('/cmd') and '$' in content:
            chatname=updates[-1]['message']['from_user']['username']
